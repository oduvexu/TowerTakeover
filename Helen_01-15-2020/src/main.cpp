...
int main() {
  ...
  while (1) {
    ...
    if(Controller.ButtonA.pressing()){
      ...
      Controller.Screen.print("%d  ", targetDist);
     autonomous();  
      }else{
        usercontrol();
      }
...
void findTargetsimple(){
  width = 0;
Camera1.takeSnapshot(GreenCube);
if(Camera1.largestObject.exists && Camera1.largestObject.width > width){
  width = Camera1.largestObject.width;
  targetColor = 3;
}
Camera1.takeSnapshot(OrangeCube);
if(Camera1.largestObject.exists && Camera1.largestObject.width > width){
