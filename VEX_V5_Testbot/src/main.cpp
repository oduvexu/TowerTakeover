...
//progress for today
//created localization class, added sonar sensors, and created functions to track the robots location based on wheel spin
...
class location{
  ...
  void setHome(){
    rotate = 0;
    xoffset = right.distance(distanceUnits::in)+5.5;
  ...
  void updatePosition(){
    ...
    if(LF1>LF2 && RF1 > RF2 && LB1>LB2 && RB1 > RB2){      //if it is going straight
      
      y = (2*((LB1*3.141529)/180))+yoffset;
    ...
    if(LF1 > LF2 && RF1 < RF2 && LB1 > LB2 && RB1 < RB2){  //if it is turning right
      
      rotate = rotate + (LF1-LF2)/6;
      
    }
    if(LF1 < LF2 && RF1 > RF2 && LB1 < LB2 && RB1 > RB2){  //if it is turning left
...
int main() {
  ...
  if(Controller.ButtonY.pressing()){
   ...
   DriveRB.setRotation(0, vex::rotationUnits::deg);
    ...
    Brain.Screen.print("Left rotation: %f          ", DriveLB.rotation(vex::rotationUnits::deg));
    Brain.Screen.setCursor(7, 0);
    Brain.Screen.print("Right rotation: %f          ", DriveRB.rotation(vex::rotationUnits::deg));
