...
class cube{
 ...
 int getDistance(){                    //calculates the distance from the camera to the cube
  ...
  int getProximity(){      // not done yet
   cubeProx = 1;           //calculate the number of cubes around it so that that area will have a higher overall priority
   return cubeProx;
  }
 ...
 int getPriority(){
   ...
   switch(cubeColor){                               //will set Ccolor to either 1, 2, or 3 based on what the user has set the priorities of each color to be
    ...
    cubePriority = (distMultiplier/getDistance() + (Ccolor * colorMultiplier));   
...
targetFound = false;
  ...
  for(i=0; i<=9; i++){                               //cycles through each color and sets the cube to the target if it has the highest priority
    if(pcubes[i].getPriority()>=currentPriority){
     if(pcubes[i].exists()){
   ...
   if(ocubes[i].getPriority()>=currentPriority){
      if(ocubes[i].exists()){
    ...
    if(gcubes[i].getPriority()>=currentPriority){
       if(gcubes[i].exists()){
