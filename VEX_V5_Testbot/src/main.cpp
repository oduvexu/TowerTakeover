...
class location{
  ...
  void updatePosition(){
    ...
    RB2 = RB1;
    RF2 = RF1;
    LB1 = DriveLB.rotation(vex::rotationUnits::deg)*0.0174533; //angles in radians
    LF1 = DriveLF.rotation(vex::rotationUnits::deg)*0.0174533;
    RB1 = DriveRB.rotation(vex::rotationUnits::deg)*0.0174533;
    RF1 = DriveRF.rotation(vex::rotationUnits::deg)*0.0174533;
...
class cube{
 ...
 int getDistance(){                     //calculates the distance from the camera to the cube
    ...
    if(Camera.objects[cubeid].width<Camera.objects[cubeid].width){
      i = Camera.objects[cubeid].width;
    }else{
      i = Camera.objects[cubeid].height;
    }
    Dist = 1655/i;
    //Dist = 1026*pow(i,-.876);
    return Dist;
...
if(robot.getAngle()<180){
  ...
  if(Controller.ButtonA.pressing()){
   ...
   DriveRB.setRotation(0, vex::rotationUnits::deg);
    ...
    Brain.Screen.print("distance: %f          ", robot.s);
  }
}
...
void turntocube(){      
 if(targetFound == false && turnDone == false){   //turns to the target cube, then sets turndone to true so that it will go to the cube
   switch(targetColor){ 
     case 3: 
        Camera.takeSnapshot(GreenCube);
     break;
  ...
  if(targetFound == false && turnDone == true){   //goes forward to the target cube and turns slightly in order to tune the center of the cube to the center of the cube
    switch(targetColor){ 
      case 3: 
