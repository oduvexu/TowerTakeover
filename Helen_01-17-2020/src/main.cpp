...
int main() {
  ...
  while (1) {
       ...
       while(Reset == true){
        ...
        Controller.Screen.print("%d  ",Camera3.largestObject.centerX);
    if(Controller.ButtonA.pressing()){
      ...
      Controller.Screen.print("%d  ", targetAcquired);
...
if(Rear.distance(distanceUnits::in)<10 && targetAcquired && turnDone){
    ...
    switch(targetColor){ 
     ...
     break;
      ...
      if(s < -20){
        ...
        DriveFR.spin(fwd,  abs(s/3),pct);
        DriveBL.spin(fwd, s/3,pct);
        DriveBR.spin(fwd,  abs(s/3),pct);
      }else
      if(s > 20){
        DriveFL.spin(fwd, s/3,pct);
